...
const Register = () => {
  ...
  const checkUsernameExists = async (username) => {
    try {
      ...
      const response = await fetch(`${CONFIG.API_URL}users/check-username?username=${username}`, {
        method: 'GET',
        headers: {
      ...
      return data.exists; // 假设API返回{exists: true/false}
    } catch (error) {
      console.error('Error checking username:', error);
      return false; // 如果发生错误，假设用户名不存在
    } finally {
